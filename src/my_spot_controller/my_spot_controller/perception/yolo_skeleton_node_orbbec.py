...
class YoloSkeletonNodeOrbbec(Node):
    ...
    def __init__(self):
        ...
        self.sub_color = self.create_subscription(
            Image, "/camera/color/image_raw", self.cb_color, 10)  # ← Cambiato

        self.sub_depth = self.create_subscription(
            Image, "/camera/depth/image_raw", self.cb_depth, 10)  # ← Cambiato (depth raw, ma allineata dal driver)

        self.sub_info = self.create_subscription(
            CameraInfo, "/camera/color/camera_info", self.cb_info, 10)  # ← Cambiato
    ...
    def adaptive_Q(self, kf, joint_idx):
        ...
        if joint_idx in {5, 6, 11, 12}:          # self.TORSO
            part_factor = 0.7
        elif joint_idx in {7, 8, 9, 10}:         # self.ARMS
            part_factor = 1.2
        elif joint_idx in {13, 14, 15, 16}:      # self.LEGS
            part_factor = 1.4
        elif joint_idx == 0:                     # self.NOSE
            part_factor = 1.8
        else:
            part_factor = 1.0
    ...
    def cb_color(self, msg):
        ...
        for i in range(self.num_joints):
            ...
            if i in self.LEGS:
                damping = 0.5  # Gambe: meno damping
            elif i in self.ARMS:
                damping = 0.4  # Braccia: poco damping
            elif i in self.TORSO:
                damping = 0.2  # self.TORSO: molto damping
            else:
            ...
            if self.kf[i].initialized:
                ...
                # Threshold differenziato
                if i in self.LEGS:
                    threshold = 3.5  # Gambe: più permissivo
                else:
                    threshold = 2.5
    ...
    def predict_only(self, stamp):
        """
        Chiama solo predict su tutti i filtri e pubblica.
        Usato quando non ci sono detection YOLO.
        """
        pts = []
        for i, k in enumerate(self.kf):
            # Occhi/orecchie: salta
            if i in {1, 2, 3, 4}:
                pts.append(None)
                continue
            ...
            if k.initialized:
                ...
                # Damping differenziato per tipo di joint
                if i in {13, 14, 15, 16}:  # LEGS
                    damping = 0.5
        ...
        # Marca tutti i joints come non visibili (predetti)
        self.visible = [False] * self.num_joints
        
        # Verifica che ci siano punti validi prima di pubblicare
        valid_pts = [p for p in pts if p is not None]
        if len(valid_pts) < 4:
            self.get_logger().debug("predict_only: troppo pochi punti validi, skip publish")
            # Pubblica comunque array vuoti per mantenere il topic attivo
...
# ============================================================
    ...
    def publish_empty(self, stamp):
        """
        Pubblica PoseArray e MarkerArray vuoti quando non ci sono dati validi.
        Elimina tutti i marker visualizzati in precedenza.
        """
        # PoseArray vuoto
        ...
        # MarkerArray con DELETE per pulire tutti i marker precedenti
        empty_marker_array = MarkerArray()
        
        # DELETE marker per joints visibili (RED)
        delete_vis = Marker()
        delete_vis.header.stamp = stamp
        delete_vis.header.frame_id = "camera_color_optical_frame"
        delete_vis.ns = "joints_visible"
        delete_vis.id = 0
        delete_vis.action = Marker.DELETE
        empty_marker_array.markers.append(delete_vis)
        
        # DELETE marker per joints predetti (BLUE)
        delete_pred = Marker()
        delete_pred.header.stamp = stamp
        delete_pred.header.frame_id = "camera_color_optical_frame"
        delete_pred.ns = "joints_predicted"
        delete_pred.id = 1
        delete_pred.action = Marker.DELETE
        empty_marker_array.markers.append(delete_pred)
        
        # DELETE marker per bones (GREEN)
        delete_bones = Marker()
        delete_bones.header.stamp = stamp
        delete_bones.header.frame_id = "camera_color_optical_frame"
        delete_bones.ns = "bones"
        delete_bones.id = 2
        delete_bones.action = Marker.DELETE
        empty_marker_array.markers.append(delete_bones)
        
        self.pub_markers.publish(empty_marker_array)
        
        self.get_logger().debug("Pubblicati marker vuoti - visualizzazione pulita")
    ...
    def publish_all(self, pts, stamp):
        """
        Pubblica PoseArray e MarkerArray.
        """
        # Protezione: verifica che pts sia valida
        if pts is None or len(pts) != self.num_joints:
            self.get_logger().warn(f"publish_all: pts invalida (len={len(pts) if pts else 0}, expected={self.num_joints})")
            self.publish_empty(stamp)
            return
